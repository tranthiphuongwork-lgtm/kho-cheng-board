...
ROT_WEBHOOK   = os.getenv("ROT_WEBHOOK")  # webhook Custom Bot của nhóm kho (ngoài tổ chức); rỗng -> dùng nhóm mặc định
...
def _bot_state_set(tok, key, value):
    ...
    if not tid: 
        print("Khong thay bang Bot_State -> bo qua luu draft"); return
    # tim record co Key = key
    rid = None
